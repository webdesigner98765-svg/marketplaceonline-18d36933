...
export const AuthPromptModal = ({ open, onClose }: AuthPromptModalProps) => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      toast.error("Ndodhi një gabim gjatë hyrjes. Provo përsëri.");
    }
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md text-center">
        <DialogHeader className="space-y-4">
          ...
          <DialogTitle className="text-2xl font-display">Hyr për të postuar</DialogTitle>
          <DialogDescription className="text-base">
            Duhet të hysh me llogarinë Google për të postuar produkte në marketplace.
          </DialogDescription>
        </DialogHeader>
